...
export const HocrPageComponent: React.StatelessComponent<HocrPageProps> = (props) => {
  ...
  const safeStyle = injectDefaultPageStyle(props.userStyle);
  const pageOptions = getNodeOptions(props.node);

  return (
    <svg
      className={safeStyle.page}
      id={getNodeId(props.node, props.idSuffix)}
    ...
    >
      ...
      <g className={safeStyle.placeholders}>
        {getNodeChildrenComponents(props)}
      </g>
    </svg>
  );
}
